...
// ดึงข้อมูลบทความ
$stmt = $conn->prepare("SELECT * FROM article_view9 WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE article_view9 SET title=?, caption_main=?, article_html=?, tags=? WHERE id=?");
    $stmt->bind_param("ssssi", $title, $caption, $html, $tags, $id);
    $stmt->execute();
    $stmt->close();

    echo "<script>alert('✅ บันทึกสำเร็จ!'); window.location.href='admin_article_view9.php?id=$id';</script>";
    exit;
}
?>
...
<form method="post">
    ...
    <label>แท็ก (คั่นด้วย , เช่น ตรวจบ้าน,แอร์,AC)</label>
    <input type="text" name="tags" value="<?= htmlspecialchars($article['tags']) ?>">

    <label>เนื้อหาบทความ (HTML, รูปภาพ, วิดีโอ)</label>
...
<script>
    new FroalaEditor('#froala', {
        ...
        toolbarButtons: [
            ...
            'formatOL', 'formatUL', 'insertLink', 'insertImage', 'insertVideo', '|',
            'html', 'undo', 'redo'
        ]
    });
</script>
