...
// ดึงข้อมูลบทความ
$stmt = $conn->prepare("SELECT * FROM article_view7 WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE article_view7 SET title=?, caption_main=?, article_html=?, tags=? WHERE id=?");
    $stmt->bind_param("ssssi", $title, $caption, $html, $tags, $id);
    $stmt->execute();
    $stmt->close();

    echo "<script>alert('✅ บันทึกสำเร็จ!'); window.location.href='admin_article_view7.php?id=$id';</script>";
    exit;
}
?>
