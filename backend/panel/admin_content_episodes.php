...
if (isset($_POST['update_main'])) {
    ...
    if (!empty($_FILES['main_thumbnail']['name'])) {
        ...
        $pdo->prepare("UPDATE carousel_main_content SET title=?, description=?, thumbnail_url=? WHERE id=1")
            ->execute([$title, $desc, $thumb_url]);
    } else {
        $pdo->prepare("UPDATE carousel_main_content SET title=?, description=? WHERE id=1")
            ->execute([$title, $desc]);
    }

    header("Location: admin_content_episodes.php");
    exit;
}
...
if (isset($_POST['add_episode'])) {
    ...
    $stmt = $pdo->prepare("INSERT INTO carousel_episodes (youtube_url, title, description) VALUES (?, ?, ?)");
...
if (isset($_POST['edit_episode'])) {
    ...
    $pdo->prepare("UPDATE carousel_episodes SET title=?, description=?, youtube_url=? WHERE id=?")
...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $pdo->prepare("DELETE FROM carousel_episodes WHERE id = ?")->execute([$id]);
    header("Location: admin_content_episodes.php");
    exit;
}

// === FETCH DATA ===
$main = $pdo->query("SELECT * FROM carousel_main_content WHERE id = 1")->fetch();
$episodes = $pdo->query("SELECT * FROM carousel_episodes ORDER BY created_at DESC")->fetchAll();
?>
...
<head>
    <meta charset="UTF-8">
    <title>Admin: Manage Episodes & Main Content</title>
    <style>
        body {
...
<div class="section">
    <h2>📝 Edit Main Content (Left Side)</h2>
    <form method="post" enctype="multipart/form-data">
        ...
        <label>Main Thumbnail (leave blank to keep current)</label>
        <input type="file" name="main_thumbnail" accept="image/*">

        <?php if ($main['thumbnail_url']): ?>
            <img src="<?= $main['thumbnail_url'] ?>" alt="Main Thumbnail" style="max-width: 200px;">
        <?php endif; ?>

        <button type="submit">💾 Update Main Content</button>
    </form>
</div>
...
<div class="section">
    ...
    <form method="post">
        ...
        <input type="text" name="youtube_url" required placeholder="https://www.youtube.com/embed/VIDEO_ID">
