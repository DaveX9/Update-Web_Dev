...
if (isset($_POST['update_main'])) {
    ...
    if (!empty($_FILES['main_thumbnail']['name'])) {
        ...
        $pdo->prepare("UPDATE carousel_main_content5 SET title=?, description=?, thumbnail_url=? WHERE id=1")
            ->execute([$title, $desc, $thumb_url]);
    } else {
        $pdo->prepare("UPDATE carousel_main_content5 SET title=?, description=? WHERE id=1")
            ->execute([$title, $desc]);
    }

    header("Location: admin_content_episodes5.php");
    exit;
}
...
if (isset($_POST['add_episode'])) {
    ...
    $stmt = $pdo->prepare("INSERT INTO carousel_episodes5 (youtube_url, title, description) VALUES (?, ?, ?)");
...
if (isset($_POST['edit_episode'])) {
    ...
    $pdo->prepare("UPDATE carousel_episodes5 SET title=?, description=?, youtube_url=? WHERE id=?")
...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $pdo->prepare("DELETE FROM carousel_episodes5 WHERE id = ?")->execute([$id]);
    header("Location: admin_content_episodes5.php");
    exit;
}
