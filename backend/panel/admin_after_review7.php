...
if (isset($_GET['fetch']) && $_GET['fetch'] === 'json') {
    ...
    $review = $conn->query("SELECT * FROM after_review_developer7 WHERE id = $id")->fetch_assoc();
    ...
    $images_result = $conn->query("SELECT * FROM after_review_images7 WHERE review_id = $id");
    $images = [];
...
// Fetch data
$review = $conn->query("SELECT * FROM after_review_developer7 WHERE id = $id")->fetch_assoc();
$images = $conn->query("SELECT * FROM after_review_images7 WHERE review_id = $id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_review'])) {
    ...
    if (!empty($_FILES['banner_image']['name'])) {
        ...
        $conn->query("UPDATE after_review_developer7 SET title='$title', description='$description', banner_image='$banner' WHERE id=$id");
    } else {
        $conn->query("UPDATE after_review_developer7 SET title='$title', description='$description' WHERE id=$id");
    }

    header("Location: admin_after_review7.php");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_image'])) {
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        $conn->query("INSERT INTO after_review_images7 (review_id, image_path, alt_text) VALUES ($id, '$img', '$alt')");
    }
...
if (isset($_GET['delete_image'])) {
    ...
    $conn->query("DELETE FROM after_review_images7 WHERE id = $img_id");
    header("Location: admin_after_review7.php");
    exit;
}
?>
