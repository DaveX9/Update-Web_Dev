...
if (isset($_GET['fetch']) && $_GET['fetch'] === 'json') {
    ...
    $review = $conn->query("SELECT * FROM after_review_developer9 WHERE id = $id")->fetch_assoc();
    $review['banner_image'] = $upload_path . $review['banner_image'];
    $review['description'] = strip_tags(htmlspecialchars_decode($review['description']));


    $images_result = $conn->query("SELECT * FROM after_review_images9 WHERE review_id = $id");
    $images = [];
...
// Fetch data
$review = $conn->query("SELECT * FROM after_review_developer9 WHERE id = $id")->fetch_assoc();
$images = $conn->query("SELECT * FROM after_review_images9 WHERE review_id = $id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_review'])) {
    ...
    if (!empty($_FILES['banner_image']['name'])) {
        ...
        $conn->query("UPDATE after_review_developer9 SET title='$title', description='$description', banner_image='$banner' WHERE id=$id");
    } else {
        $conn->query("UPDATE after_review_developer9 SET title='$title', description='$description' WHERE id=$id");
    }

    header("Location: admin_after_review9.php");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_image'])) {
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        $conn->query("INSERT INTO after_review_images9 (review_id, image_path, alt_text) VALUES ($id, '$img', '$alt')");
    }

    header("Location: admin_after_review8.php");
    exit;
}
...
if (isset($_GET['delete_image'])) {
    ...
    $conn->query("DELETE FROM after_review_images9 WHERE id = $img_id");
    header("Location: admin_after_review8.php");
    exit;
}
?>
